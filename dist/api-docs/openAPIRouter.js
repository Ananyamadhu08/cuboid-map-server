...
var openAPIRouter_exports = {};
__export(openAPIRouter_exports, {
  openAPIRouter: () => openAPIRouter
});
module.exports = __toCommonJS(openAPIRouter_exports);
...
var authenticateToken = (req, res, next) => {
  ...
  if (!authHeader) {
    return res.status(import_http_status_codes6.StatusCodes.UNAUTHORIZED).json(ServiceResponse.failure("Access denied. No token provided.", null, import_http_status_codes6.StatusCodes.UNAUTHORIZED));
  }
  ...
  try {
    ...
    if (err instanceof import_jsonwebtoken2.TokenExpiredError) {
      ...
      return res.status(import_http_status_codes6.StatusCodes.UNAUTHORIZED).json(ServiceResponse.failure("Token expired.", null, import_http_status_codes6.StatusCodes.UNAUTHORIZED));
    } else if (err instanceof import_jsonwebtoken2.JsonWebTokenError) {
      console.error("Invalid token:", err.message);
      return res.status(import_http_status_codes6.StatusCodes.FORBIDDEN).json(ServiceResponse.failure("Invalid token.", null, import_http_status_codes6.StatusCodes.FORBIDDEN));
    } else {
      console.error("Token verification failed:", err);
      return res.status(import_http_status_codes6.StatusCodes.INTERNAL_SERVER_ERROR).json(ServiceResponse.failure("Token verification failed.", null, import_http_status_codes6.StatusCodes.INTERNAL_SERVER_ERROR));
    }
  }
};
...
var MapCaptureService = class {
  ...
  async saveCapture(userId, data) {
    try {
      ...
      return ServiceResponse.success("Map capture saved successfully", capture, import_http_status_codes7.StatusCodes.CREATED);
    } catch (error) {
      console.error("Error saving map capture:", error);
      return ServiceResponse.failure("Failed to save map capture.", null, import_http_status_codes7.StatusCodes.INTERNAL_SERVER_ERROR);
    }
  }
  async getAllCaptures() {
    try {
      const captures = await this.mapCaptureRepository.findAllCaptures();
      return ServiceResponse.success("Map captures retrieved successfully", captures, import_http_status_codes7.StatusCodes.OK);
    } catch (error) {
      console.error("Error retrieving map captures:", error);
      return ServiceResponse.failure("Failed to retrieve map captures.", null, import_http_status_codes7.StatusCodes.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async getCaptureById(id) {
    try {
      ...
      if (!capture) {
        return ServiceResponse.failure("Map capture not found", null, import_http_status_codes7.StatusCodes.NOT_FOUND);
      }
      return ServiceResponse.success("Map capture retrieved successfully", capture, import_http_status_codes7.StatusCodes.OK);
    } catch (error) {
      console.error("Error retrieving map capture:", error);
      return ServiceResponse.failure("Failed to retrieve map capture.", null, import_http_status_codes7.StatusCodes.INTERNAL_SERVER_ERROR);
    }
  }
  async getLatestCaptureByUserId(userId) {
    try {
      ...
      if (!latestCapture) {
        return ServiceResponse.failure("No captures found for this user.", null, import_http_status_codes7.StatusCodes.NOT_FOUND);
      }
      return ServiceResponse.success("Latest map capture retrieved successfully", latestCapture, import_http_status_codes7.StatusCodes.OK);
    } catch (error) {
      console.log("Error retrieving latest map capture:", error);
      return ServiceResponse.failure("Failed to retrieve latest map capture.", null, import_http_status_codes7.StatusCodes.INTERNAL_SERVER_ERROR);
    }
  }
  async getTopCapturedRegions(userId) {
    ...
    if (cachedData) {
      return ServiceResponse.success("Top captured regions retrieved from cache", cachedData, import_http_status_codes7.StatusCodes.OK);
    }
    try {
      ...
      return ServiceResponse.success("Top captured regions retrieved successfully", topRegions, import_http_status_codes7.StatusCodes.OK);
    } catch (error) {
...
var MapCaptureController = class {
  async saveCapture(req, res) {
    ...
    if (!userId) {
      return ServiceResponse.failure("User ID is missing from the request.", null, import_http_status_codes8.StatusCodes.BAD_REQUEST);
    }
    const { title, longitude, latitude, zoom, bearing, pitch, imageUrl } = req.body;
    if (!title || !longitude || !latitude || !imageUrl) {
      return ServiceResponse.failure("Missing required fields in the request.", null, import_http_status_codes8.StatusCodes.BAD_REQUEST);
    }
...
var MapCaptureSchema = import_zod3.z.object({
  title: import_zod3.z.string().min(1, "Title is required"),
  longitude: import_zod3.z.number(),
  latitude: import_zod3.z.number(),
  zoom: import_zod3.z.number(),
  bearing: import_zod3.z.number(),
  pitch: import_zod3.z.number(),
  imageUrl: import_zod3.z.string().url()
}).openapi("MapCaptureRequest");
var MapCaptureResponseSchema = import_zod3.z.object({
  id: import_zod3.z.string().openapi({ description: "The unique identifier of the map capture" }),
  userId: import_zod3.z.string().openapi({ description: "The ID of the user who created the capture" }),
  title: import_zod3.z.string().openapi({ description: "The title of the map capture" }),
  longitude: import_zod3.z.number(),
  latitude: import_zod3.z.number(),
  zoom: import_zod3.z.number(),
  bearing: import_zod3.z.number(),
  pitch: import_zod3.z.number(),
  imageUrl: import_zod3.z.string().url(),
  createdAt: import_zod3.z.string().openapi({ description: "Timestamp of when the capture was created" })
}).openapi("MapCaptureResponse");

// src/api/map-capture/mapCaptureRouter.ts
var mapCaptureRegistry = new import_zod_to_openapi3.OpenAPIRegistry();
var mapCaptureRouter = import_express2.default.Router();
...
var AuthRequestSchema = import_zod4.z.object({
  username: import_zod4.z.string().min(3, { message: "Username is required and must be at least 3 characters" }).openapi({ example: "user123" }),
  password: import_zod4.z.string().min(8, { message: "Password is required and must be at least 8 characters" }).openapi({ example: "securepassword" }),
  email: import_zod4.z.string().email({ message: "A valid email is required" }).openapi({ example: "user@example.com" })
}).openapi("AuthRequest");
var AuthResponseSchema = import_zod4.z.object({
  accessToken: import_zod4.z.string().openapi({ example: "jwt.token.here" }),
  refreshToken: import_zod4.z.string().openapi({ example: "refresh.token.here" }),
  username: import_zod4.z.string().openapi({ example: "user123" }),
  email: import_zod4.z.string().email().openapi({ example: "user@example.com" })
}).openapi("AuthResponse");

// src/api/auth/authRouter.ts
var authRegistry = new import_zod_to_openapi5.OpenAPIRegistry();
var authRouter = import_express4.default.Router();
...
authRegistry.registerPath({
  ...
  request: {
    body: {
      content: {
        "application/json": {
          schema: import_zod5.z.object({ refreshToken: import_zod5.z.string().openapi({ example: "refresh.token.here" }) })
        }
      }
    }
  },
  responses: createApiResponse(
    import_zod5.z.object({ accessToken: import_zod5.z.string().openapi({ example: "jwt.token.here" }) }),
    "Token refreshed"
  )
});
...
authRouter.post("/refresh-token", validateRequest(import_zod5.z.object({ refreshToken: import_zod5.z.string() })), authController.refreshToken);
...
// src/api-docs/openAPIRouter.ts
var openAPIRouter = import_express5.default.Router();
