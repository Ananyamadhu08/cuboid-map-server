...
var AuthService = class {
  async register({ username, password, email }) {
    try {
      ...
      if (error instanceof import_library.PrismaClientKnownRequestError) {
        if (error.code === "P2002") {
          ...
          return ServiceResponse.failure(
            ...
            null,
            import_http_status_codes4.StatusCodes.BAD_REQUEST
          );
        }
      }
      logger.error(errorMessage);
      return ServiceResponse.failure(
        ...
        null,
        import_http_status_codes4.StatusCodes.INTERNAL_SERVER_ERROR
      );
    }
  }
  async login({ username, password }) {
    ...
    if (!user || !await import_bcrypt2.default.compare(password, user.password)) {
      return ServiceResponse.failure("Invalid username or password", null, import_http_status_codes4.StatusCodes.UNAUTHORIZED);
    }
    const accessToken = generateToken(user.id);
    const refreshToken = generateRefreshToken(user.id);
    await authRepository.saveRefreshToken(user.id, refreshToken);
  ...
  async refresh(refreshToken) {
    try {
      ...
      if (!user) {
        return ServiceResponse.failure("User not found", null, import_http_status_codes4.StatusCodes.UNAUTHORIZED);
      }
      const storedRefreshToken = await authRepository.findRefreshToken(userId);
      if (refreshToken !== storedRefreshToken) {
        return ServiceResponse.failure("Invalid refresh token", null, import_http_status_codes4.StatusCodes.FORBIDDEN);
      }
      ...
      if (error instanceof import_jsonwebtoken2.TokenExpiredError) {
        return ServiceResponse.failure("Refresh token expired", null, import_http_status_codes4.StatusCodes.FORBIDDEN);
      } else if (error instanceof import_jsonwebtoken2.JsonWebTokenError) {
        return ServiceResponse.failure("Invalid refresh token", null, import_http_status_codes4.StatusCodes.FORBIDDEN);
      } else if (error instanceof Error) {
        logger.error("Error refreshing token:", error.message, error.stack);
        return ServiceResponse.failure("Error processing refresh token", null, import_http_status_codes4.StatusCodes.INTERNAL_SERVER_ERROR);
      } else {
...
var authenticateToken = (req, res, next) => {
  ...
  if (!authHeader) {
    return res.status(import_http_status_codes5.StatusCodes.UNAUTHORIZED).json(ServiceResponse.failure("Access denied. No token provided.", null, import_http_status_codes5.StatusCodes.UNAUTHORIZED));
  }
  ...
  try {
    ...
    if (err instanceof import_jsonwebtoken3.TokenExpiredError) {
      console.error("Token expired:", err.message);
      return res.status(import_http_status_codes5.StatusCodes.UNAUTHORIZED).json(ServiceResponse.failure("Token expired.", null, import_http_status_codes5.StatusCodes.UNAUTHORIZED));
    } else if (err instanceof import_jsonwebtoken3.JsonWebTokenError) {
      console.error("Invalid token:", err.message);
      return res.status(import_http_status_codes5.StatusCodes.FORBIDDEN).json(ServiceResponse.failure("Invalid token.", null, import_http_status_codes5.StatusCodes.FORBIDDEN));
    } else {
      console.error("Token verification failed:", err);
      return res.status(import_http_status_codes5.StatusCodes.INTERNAL_SERVER_ERROR).json(ServiceResponse.failure("Token verification failed.", null, import_http_status_codes5.StatusCodes.INTERNAL_SERVER_ERROR));
    }
  }
};
...
var MapCaptureRepository = class {
  async saveCapture(data) {
    return prisma2.mapCapture.create({
      data
    });
  }
  async findAllCaptures() {
    return prisma2.mapCapture.findMany();
  }
  ...
  async findCaptureById(id) {
    return prisma2.mapCapture.findUnique({
      where: { id }
    });
  }
  async findLatestCaptureByUserId(userId) {
    return prisma2.mapCapture.findFirst({
      where: { userId },
      orderBy: {
        createdAt: "desc"
      }
  ...
  async findTopCapturedRegions(userId) {
    const result = await prisma2.mapCapture.groupBy({
...
var MapCaptureService = class {
  ...
  async saveCapture(userId, data) {
    try {
      ...
      return ServiceResponse.success("Map capture saved successfully", capture, import_http_status_codes6.StatusCodes.CREATED);
    } catch (error) {
      console.error("Error saving map capture:", error);
      return ServiceResponse.failure("Failed to save map capture.", null, import_http_status_codes6.StatusCodes.INTERNAL_SERVER_ERROR);
    }
  }
  async getAllCaptures() {
    try {
      const captures = await this.mapCaptureRepository.findAllCaptures();
      return ServiceResponse.success("Map captures retrieved successfully", captures, import_http_status_codes6.StatusCodes.OK);
    } catch (error) {
      console.error("Error retrieving map captures:", error);
      return ServiceResponse.failure("Failed to retrieve map captures.", null, import_http_status_codes6.StatusCodes.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async getCaptureById(id) {
    try {
      ...
      if (!capture) {
        return ServiceResponse.failure("Map capture not found", null, import_http_status_codes6.StatusCodes.NOT_FOUND);
      }
      return ServiceResponse.success("Map capture retrieved successfully", capture, import_http_status_codes6.StatusCodes.OK);
    } catch (error) {
      console.error("Error retrieving map capture:", error);
      return ServiceResponse.failure("Failed to retrieve map capture.", null, import_http_status_codes6.StatusCodes.INTERNAL_SERVER_ERROR);
    }
  }
  async getLatestCaptureByUserId(userId) {
    try {
      ...
      if (!latestCapture) {
        return ServiceResponse.failure("No captures found for this user.", null, import_http_status_codes6.StatusCodes.NOT_FOUND);
      }
      return ServiceResponse.success("Latest map capture retrieved successfully", latestCapture, import_http_status_codes6.StatusCodes.OK);
    } catch (error) {
      console.log("Error retrieving latest map capture:", error);
      return ServiceResponse.failure("Failed to retrieve latest map capture.", null, import_http_status_codes6.StatusCodes.INTERNAL_SERVER_ERROR);
    }
  }
  async getTopCapturedRegions(userId) {
    ...
    if (cachedData) {
      return ServiceResponse.success("Top captured regions retrieved from cache", cachedData, import_http_status_codes6.StatusCodes.OK);
    }
    try {
      ...
      return ServiceResponse.success("Top captured regions retrieved successfully", topRegions, import_http_status_codes6.StatusCodes.OK);
    } catch (error) {
...
var MapCaptureController = class {
  async saveCapture(req, res) {
    ...
    if (!userId) {
      return ServiceResponse.failure("User ID is missing from the request.", null, import_http_status_codes7.StatusCodes.BAD_REQUEST);
    }
    const { title, longitude, latitude, zoom, bearing, pitch, imageUrl } = req.body;
    if (!title || !longitude || !latitude || !imageUrl) {
      return ServiceResponse.failure("Missing required fields in the request.", null, import_http_status_codes7.StatusCodes.BAD_REQUEST);
    }
...
var unexpectedRequest = (_req, res) => {
  res.sendStatus(import_http_status_codes8.StatusCodes.NOT_FOUND);
};
...
var customLogLevel = (_req, res, err) => {
  if (err || res.statusCode >= import_http_status_codes9.StatusCodes.INTERNAL_SERVER_ERROR) return "error" /* Error */;
  if (res.statusCode >= import_http_status_codes9.StatusCodes.BAD_REQUEST) return "warn" /* Warn */;
  if (res.statusCode >= import_http_status_codes9.StatusCodes.MULTIPLE_CHOICES) return "silent" /* Silent */;
  return "info" /* Info */;
};
var customSuccessMessage = (req, res) => {
  if (res.statusCode === import_http_status_codes9.StatusCodes.NOT_FOUND) return (0, import_http_status_codes9.getReasonPhrase)(import_http_status_codes9.StatusCodes.NOT_FOUND);
