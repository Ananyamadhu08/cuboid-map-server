...
var mapCaptureService_exports = {};
__export(mapCaptureService_exports, {
  MapCaptureService: () => MapCaptureService,
  mapCaptureService: () => mapCaptureService
});
module.exports = __toCommonJS(mapCaptureService_exports);
...
var MapCaptureService = class {
  ...
  async saveCapture(userId, data) {
    try {
      ...
      return ServiceResponse.success("Map capture saved successfully", capture, import_http_status_codes2.StatusCodes.CREATED);
    } catch (error) {
      console.error("Error saving map capture:", error);
      return ServiceResponse.failure("Failed to save map capture.", null, import_http_status_codes2.StatusCodes.INTERNAL_SERVER_ERROR);
    }
  }
  async getAllCaptures() {
    try {
      const captures = await this.mapCaptureRepository.findAllCaptures();
      return ServiceResponse.success("Map captures retrieved successfully", captures, import_http_status_codes2.StatusCodes.OK);
    } catch (error) {
      console.error("Error retrieving map captures:", error);
      return ServiceResponse.failure("Failed to retrieve map captures.", null, import_http_status_codes2.StatusCodes.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async getCaptureById(id) {
    try {
      ...
      if (!capture) {
        return ServiceResponse.failure("Map capture not found", null, import_http_status_codes2.StatusCodes.NOT_FOUND);
      }
      return ServiceResponse.success("Map capture retrieved successfully", capture, import_http_status_codes2.StatusCodes.OK);
    } catch (error) {
      console.error("Error retrieving map capture:", error);
      return ServiceResponse.failure("Failed to retrieve map capture.", null, import_http_status_codes2.StatusCodes.INTERNAL_SERVER_ERROR);
    }
  }
  async getLatestCaptureByUserId(userId) {
    try {
      ...
      if (!latestCapture) {
        return ServiceResponse.failure("No captures found for this user.", null, import_http_status_codes2.StatusCodes.NOT_FOUND);
      }
      return ServiceResponse.success("Latest map capture retrieved successfully", latestCapture, import_http_status_codes2.StatusCodes.OK);
    } catch (error) {
      console.log("Error retrieving latest map capture:", error);
      return ServiceResponse.failure("Failed to retrieve latest map capture.", null, import_http_status_codes2.StatusCodes.INTERNAL_SERVER_ERROR);
    }
  }
  async getTopCapturedRegions(userId) {
    ...
    if (cachedData) {
      return ServiceResponse.success("Top captured regions retrieved from cache", cachedData, import_http_status_codes2.StatusCodes.OK);
    }
    try {
      ...
      return ServiceResponse.success("Top captured regions retrieved successfully", topRegions, import_http_status_codes2.StatusCodes.OK);
    } catch (error) {
...
//# sourceMappingURL=mapCaptureService.js.map